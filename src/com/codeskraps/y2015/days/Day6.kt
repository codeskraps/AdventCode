...
class Day6 : Day() {
    ...
    override fun partOne(): String {
        val lights = Lights()
        INPUT.forEach {
            lights.executePartOne(Instruction.parse(it))
        }
        return lights.countOn().toString()
    }
    ...
    class Lights {
        private var l = arrayOf<Array<Int>>()
        ...
        fun executePartOne(instruction: Instruction) {
            val r1 = if (instruction.a < instruction.x) instruction.a else instruction.x
            val r2 = if (instruction.a < instruction.x) instruction.x else instruction.a
            val c1 = if (instruction.b < instruction.y) instruction.b else instruction.y
            val c2 = if (instruction.b < instruction.y) instruction.y else instruction.b

            for (r in r1..r2) {
                for (c in c1..c2) {
                    when (instruction.order) {
                        "on" -> l[r][c]++
                        "off" -> if (l[r][c] > 0) l[r][c]--
                        "toggle" -> l[r][c] += 2
                    }
                }
            }
    ...
    class Instruction(val order: String, val a: Int, val b: Int, val x: Int, val y: Int) {

        companion object {
            fun parse(entry: String): Instruction {

                val order: String = if (entry.startsWith("turn")) {
                    entry.substring(5, 8).trim()
                } else "toggle"
                ...
                val a: Int = entry.substring(index, entry.indexOf(",")).trim().toInt()
                val b: Int = entry.substring(entry.indexOf(",") + 1, entry.indexOf("through")).trim().toInt()
                val x: Int = entry.substring(entry.indexOf("through") + 7, entry.lastIndexOf(',')).trim().toInt()
                val y: Int = entry.substring(entry.lastIndexOf(',') + 1, entry.length).trim().toInt()

                return Instruction(order, a, b, x, y)
            }
        }
    }

    companion object {
        private val INPUT1 = arrayOf("turn on 0,0 through 999,999")
        private val INPUT2 = arrayOf("toggle 0,0 through 999,0")
        private val INPUT3 = arrayOf("turn on 499,499 through 500,500")
