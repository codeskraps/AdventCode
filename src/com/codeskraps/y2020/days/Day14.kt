...
class Day14 : Day() {
    ...
    init {
        ...
        INPUT.forEach {
            val a = it.split("=")
            if (a[0].trim() == "mask") {
                instruction = Instruction(a[1].trim())
                instructions.add(instruction)
            } else {
                val address = it.substring(4, it.indexOf("]"))
                val entry = it.substring(it.indexOf("=") + 2, it.length)
                val memoryEntry = MemoryEntry(address.toInt(), entry.toBigInteger())
                instruction.memoryEntries.add(memoryEntry)
            }
        }
    }
    ...
    override fun partOne(): String {
        ...
        instructions.forEach { int ->
            int.memoryEntries.forEach {
                map[it.address] = write(int.mask, it.entry)
            }
        }

        map.forEach { (_, u) -> count += u }

        return count.toString()
    }

    private fun write(mask: String, entry: BigInteger): BigInteger {
        val array = mask.toCharArray()
        val binary = entry.toString(2).reversed()

        binary.forEachIndexed { index, bit ->
            if (array[array.size - 1 - index] == 'X') {
                array[array.size - 1 - index] = bit
            }
        }

        return BigInteger(String(array).replace('X', '0'), 2)
    }
    ...
    private fun write(mask: String, memoryEntry: MemoryEntry, map: MutableMap<BigInteger, BigInteger>) {
        var binary = memoryEntry.address.toString(2)

        while (binary.length < 36) {
            binary = "0$binary"
        }

        val array = binary.toCharArray()

        mask.reversed().forEachIndexed { index, bit ->
            when (bit) {
                '1' -> array[array.size - 1 - index] = '1'
                'X' -> array[array.size - 1 - index] = 'X'
            }
        }
        ...
        addresses.forEach {
            map[BigInteger(it, 2)] = memoryEntry.entry
        }
    }

    private fun addresses(address: String, addresses: MutableList<String>) {
        if (address.contains('X')) {
            val index = address.indexOf('X')
            val newAddress = address.toCharArray()

            newAddress[index] = '0'
            addresses(String(newAddress), addresses)
    ...
    data class Instruction(val mask: String) {
        val memoryEntries = ArrayList<MemoryEntry>()
    }

    data class MemoryEntry(val address: Int, val entry: BigInteger)
