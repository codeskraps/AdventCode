...
class Day6 : Day() {
    ...
    override fun partOne(): String {
        val all = mutableListOf<Set<Char>>()
        var yes = mutableSetOf<Char>()
        ...
        INPUT.forEach {
            ...
            if (it.isEmpty()) {
                ...
                it.forEach { a -> yes.add(a) }
            }
        }

        var count = 0
        all.forEach { count += it.size }
    ...
    private fun findSame(ans: List<String>): Int {
        var count = 0

        ans[0].forEach {
            var has = true
            ans.forEach { a -> if (!a.toCharArray().contains(it)) has = false }
            if (has) count++
        }
