...
class Day7 : Day() {
    private val rules = ArrayList<Bag>()

    init {
        INPUT.forEach {
            val c = it.split(",")
            val b = c[0].split("contain")
            val bag = Bag()
            bag.color = b[0].replace("bags", "").trim()
            bag.contain.add(parseBag(b[1]))

            for (i in 1 until c.size) {
                bag.contain.add(parseBag(c[i]))
            }
    ...
    private fun parseBag(value: String): Bag {
        val bag = Bag()

        val d = value.trim().split(" ")
        if (d[0] != "no") {
            bag.quantity = d[0].toInt()
            bag.color = d[1] + " " + d[2]
        }
    ...
    override fun partOne(): String = rules.filter { traverseBags(it) }.size.toString()

    private fun traverseBags(bag: Bag): Boolean {
        var has = false

        for (child in bag.contain) {
            if (child.color == "shiny gold") has = true

            if (child != bag) {
                rules.filter { it.color == child.color }.forEach { if (traverseBags(it)) has = true }
            }
        }
    ...
    override fun partTwo(): String = (countBags(rules.first { it.color == "shiny gold" }) - 1).toString()

    private fun countBags(bag: Bag): Int {
        var count = 1

        for (child in bag.contain) {
            var childCount = child.quantity
            if (child != bag) {
                val filter = rules.filter { it.color == child.color }
                if (filter.isNotEmpty()) childCount *= countBags(filter.first())
            }
            count += childCount
        }
        return count
    ...
    class Bag {
        var color: String = ""
        var quantity: Int = 0
        var contain: ArrayList<Bag> = ArrayList()
    }
