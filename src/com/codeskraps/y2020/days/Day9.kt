...
class Day9 : Day() {
    ...
    override fun partOne(): String {
        ...
        for (i in PREAMBLE until INPUT.size) {
            var valid = false
            for (x in i - PREAMBLE until i - 1) {
                for (y in x + 1 until i) {
            ...
            if (!valid) return INPUT[i].toString()
        }
    ...
    private fun slice(x: Int, y: Int): List<Long> {
        val slice = arrayListOf<Long>()
        for (i in x..y) {
            slice.add(INPUT[i])
        }
        return slice
