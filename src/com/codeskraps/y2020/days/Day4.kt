...
class Day4 : Day() {
        ...
        INPUT.forEach {
            if (it.isEmpty()) {
                ...
                values.forEach { v ->
                    val c = v.split(":")
                    when (c[0]) {
                        "byr" -> passport.byr = c[1].toInt()
                        "iyr" -> passport.iyr = c[1].toInt()
                        "eyr" -> passport.eyr = c[1].toInt()
                        "hgt" -> passport.hgt = c[1]
                        "hcl" -> passport.hcl = c[1]
                        "ecl" -> passport.ecl = c[1]
                        "pid" -> passport.pid = c[1]
                        "cid" -> passport.cid = c[1].toInt()
                    }
                }
            }
        }
    ...
    override fun partOne(): String {
        ...
        passports.forEach { if (it.isValidPartOne()) answer++ }
    ...
    override fun partTwo(): String {
        ...
        passports.forEach { if (it.isValidPartTwo()) answer++ }
    ...
    class Passport {
        ...
        fun isValidPartOne(): Boolean {
            ...
            if (hgt.isEmpty()) return false
            if (hcl.isEmpty()) return false
            if (ecl.isEmpty()) return false
        ...
        fun isValidPartTwo(): Boolean {
            ...
            if (hgt.endsWith("cm")) {
                val cm = hgt.substring(0, hgt.indexOf("cm")).toInt()
                if (cm < 150 || cm > 193) return false
            } else if (hgt.endsWith("in")) {
                val inc = hgt.substring(0, hgt.indexOf("in")).toInt()
                if (inc < 59 || inc > 76) return false
            } else return false

            if (!hcl.startsWith("#")) return false
            else if (hcl.length != 7) return false
            else hcl.substring(1, hcl.length).forEach { if (!it.isLetterOrDigit()) return false }
            ...
            if (
                    ...
                    ecl != "grn" &&
                    ecl != "hzl" &&
                    ecl != "oth") return false

            if (pid.length != 9) return false
            pid.forEach { if (!it.isDigit()) return false }
