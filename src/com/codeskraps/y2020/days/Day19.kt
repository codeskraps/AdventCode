...
class Day19 : Day() {
    ...
    private val valid = LinkedHashMap<Int, Rule>()
    ...
    init {
        ...
        while (INPUT[index].isNotEmpty()) {
            ...
            rule = when {
                split[1].contains("|") -> {
                    val sr = split[1].split("|")

                    val sr0 = sr[0].trim().split(" ")
                    val pipe1 = if (sr0.size == 2) Double(sr0[0].toInt(), sr0[1].toInt())
                    else Simple(sr0[0].toInt())

                    val sr1 = sr[1].trim().split(" ")
                    val pipe2 = if (sr1.size == 2) Double(sr1[0].toInt(), sr1[1].toInt())
                    else Simple(sr1[0].toInt())
                ...
                split[1][1].isLetter() -> {
                    ...
                    val sr = split[1].trim().split(" ")
                    if (sr.size == 2) Double(sr[0].toInt(), sr[1].toInt())
                    else Simple(sr[0].toInt())
                }
            }

            valid[split[0].toInt()] = rule

            index++
        }

        index++ // skip empty string
        while (index < INPUT.size) {
            received.add(INPUT[index])
            index++
        }
    }

    override fun partOne(): String {
        val regex = buildRegexPartOne(valid[0]!!)
        return received.filter { it.matches(Regex(regex)) }.size.toString()
    }

    private fun buildRegexPartOne(rule: Rule): String {
        return when (rule) {
            is Final -> rule.value.toString()
            is Simple -> buildRegexPartOne(valid[rule.value]!!)
            is Double -> buildRegexPartOne(valid[rule.value1]!!) + buildRegexPartOne(valid[rule.value2]!!)
            is Pipped -> "(" + buildRegexPartOne(rule.pipe1) + "|" + buildRegexPartOne(rule.pipe2) + ")"
            else -> "!!"
        }
    }

    override fun partTwo(): String {
        valid[8] = Pipped(Simple(42), Double(42, 8))
        valid[11] = Pipped(Double(42, 31), Triple(42, 11, 31))
        ...
        while (depth < 10) {
            val regex = buildRegexPartTwo(valid[0]!!, 0, depth)
            val match = left.filter { it.matches(Regex(regex)) }.size
            left = ArrayList(left.filter { !it.matches(Regex(regex)) })
            final += match
            depth++
        }
    ...
    private fun buildRegexPartTwo(rule: Rule, depth: Int, max: Int): String {
        var d = depth
        ...
        return when (rule) {
            ...
            is Simple -> {
                ...
                if (d >= max) "x" else buildRegexPartTwo(valid[rule.value]!!, d, max)
            }
            is Double -> {
                if (rule.value1 == 8 || rule.value2 == 8) d++
                if (rule.value1 == 11 || rule.value2 == 11) d++

                val regexOne = if (d >= max && (rule.value1 == 8 || rule.value1 == 11)) "x" else buildRegexPartTwo(valid[rule.value1]!!, d, max)
                val regexTwo = if (d >= max && (rule.value2 == 8 || rule.value2 == 11)) "x" else buildRegexPartTwo(valid[rule.value2]!!, d, max)

                regexOne + regexTwo
            }
            is Triple -> {
                if (rule.value1 == 8 || rule.value2 == 8 || rule.value3 == 8) d++
                if (rule.value1 == 11 || rule.value2 == 11 || rule.value3 == 11) d++

                val regexOne = if (d >=max && (rule.value1 == 8 || rule.value1 == 11)) "x" else buildRegexPartTwo(valid[rule.value1]!!, d, max)
                val regexTwo = if (d >= max && (rule.value2 == 8 || rule.value2 == 11)) "x" else buildRegexPartTwo(valid[rule.value2]!!, d, max)
                val regexThree = if (d >= max && (rule.value3 == 8 || rule.value3 == 11)) "x" else buildRegexPartTwo(valid[rule.value3]!!, d, max)

                regexOne + regexTwo + regexThree
            }
            is Pipped -> "(" + buildRegexPartTwo(rule.pipe1, depth, max) + "|" + buildRegexPartTwo(rule.pipe2, depth, max) + ")"
            else -> "!!"
        }
    }
    ...
    class Final(val value: Char) : Rule
    class Simple(val value: Int) : Rule
    class Double(val value1: Int, val value2: Int) : Rule
    class Triple(val value1: Int, val value2: Int, val value3: Int) : Rule
    class Pipped(val pipe1: Rule, val pipe2: Rule) : Rule
