...
class Day2 : Day() {
    private val passwords: ArrayList<Password> = ArrayList()
    ...
    init {
        for (entry in INPUT) {
            ...
            val min = part.substring(0, part.indexOf("-")).toInt()
            val max = part.substring(part.indexOf("-") + 1).toInt()

            part = parts[1].trim()
            val letter = part.substring(0, part.indexOf(":")).toCharArray()[0]

            val password = parts[2].trim()

            passwords.add(Password(min, max, letter, password))
        }
    }
    ...
    override fun partOne(): String {
        ...
        for (password in passwords) {
            val letters = password.entry.toCharArray()
            var total = 0

            for (letter in letters) {
                if (letter == password.letter) total++
            }

            if (total >= password.min && total <= password.max) {
                debugPrintf("%s\n", password)
                answer++
            }
        }
    ...
    override fun partTwo(): String {
        ...
        for (password in passwords) {
            ...
            if (letters[password.min - 1] == password.letter && letters[password.max - 1] != password.letter) {
                debugPrintf("%s\n", password)
                answer++

            } else if (letters[password.max - 1] == password.letter && letters[password.min - 1] != password.letter) {
                debugPrintf("%s\n", password)
                answer++
            }
        }
    ...
    private class Password(val min: Int, val max: Int, val letter: Char, val entry: String) {
